//// Learn both diagnosed and undiagnosed prevalence from GUMCAD
...
transformed parameters {
    ...
    if (is_nan(pinodelta[NGMSM]) || pinodelta[NGMSM] > 1){
	reject("pinodelta[NGMSM] must be <= 1");
    }
    ...
    pi[GMSM] = pinodelta[GMSM] + pidelta[GMSM];
    delta[GMSM] = pidelta[GMSM] / pi[GMSM];
    for(grp in NGMSM:PMSM)// Subgroup counter ("GMSM", "NGMSM", "PMSM")
	{
	...
	pi[MSM] = dot_product(rho[GMSM:PMSM], pi[GMSM:PMSM])  / rho_msm;
        ...
        if (pi[GMSM] > 1) {
	    reject("pi[GMSM] must be <=1");
	}
	if (is_nan(pi[NGMSM]) || (pi[NGMSM] > 1)){
	    print("DEBUG: age=",", pinodelta=",pinodelta[NGMSM], ", 1-delta=",1 - delta[NGMSM]);
	}
	if (is_nan(pi[GMSM])){
	    print("DEBUG: pin=", pinodelta[GMSM], "1-d=", 1 - delta[GMSM]);
	}
	if (pi[PMSM] > 1) {
	    reject("pi[PMSM] must be <= 1");
	}
